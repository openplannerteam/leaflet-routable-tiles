...
L.VectorGrid.RoutableTiles = L.VectorGrid.extend({
    ...
    _getVectorTilePromise: function (coords, tileBounds) {
        ...
        return jsonld.promises.compact(tileUrl, context).then(function (json) {
            ...
            //geojsonvt outputs tags properties cfr. the vector spec, yet this library needs properties instead
            let vtfeatures = geojsonvt(feats).getTile(14, coords.x, coords.y).features.map((item) => {
                return {
                    type: item.type,
